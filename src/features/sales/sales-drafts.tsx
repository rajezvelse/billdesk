...
class SalesDrafts extends ReactComponent<any, {
  ...
  loadDrafts = () => {
    this.context.electronIpc.once("getSalesDraftsResponse", (event: IpcRendererEvent, status: number, response: any) => {
    ...
    this.context.electronIpc.send("getSalesDrafts");
  }
    ...
    return (<>
      ...
      {this.state.loadError ? <InfoError>{this.state.loadError}</InfoError> : <>
        <DraftsDrawerContainer>
          ...
          <CardContent>
            <CardSectionTitle gutterBottom variant="h5">Sales drafts</CardSectionTitle>

            <ItemsList>
              {this.state.drafts.map((draft: any, index: any) =>
                <ListItem key={uniqueId()} onClick={() => this.onDraftSelect(index)}>
                  <ListItemText primary={draft.customer.name} secondary={<FormatDate value={draft.date} format={'DD/MM/YYYY, hh:mm a'} />}></ListItemText>
                </ListItem>
              )}
            </ItemsList>
...
SalesDrafts.contextType = RootContext;

export default SalesDrafts;
