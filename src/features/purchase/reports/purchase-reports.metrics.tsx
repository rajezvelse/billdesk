...
import PurchaseDatewiseChart from './purchase-datewise-chart';


class PurchaseReportsMetrics extends ReactComponent<{
  startDate?: Date;
  endDate?: Date;
}, {
  brands: any[];
  ...
  products: any[];
  netSummary: any;
  dateWisePurchasePaymentsSummary: any;
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      products: [],
      netSummary: null,
      dateWisePurchasePaymentsSummary: null,
  ...
  fetchFilterData = () => {
    this.context.electronIpc.once("fetchPurchaseReportFilterDataForMetricsResponse", (event: IpcRendererEvent, status: number, response: any) => {
    ...
    this.context.electronIpc.send("fetchPurchaseReportFilterDataForMetrics");
  }

  fetchData = () => {
    this.context.electronIpc.once("fetchPurchaseReportsMetricsResponse", (event: IpcRendererEvent, status: number, response: any) => {
      this.context.setLoading(false);
      ...
      this.setState({
        ...
        dateWisePurchasePaymentsSummary: response.dateWisePurchasePaymentsSummary,
    ...
    this.context.setLoading(true);
    this.context.electronIpc.send("fetchPurchaseReportsMetrics", {
    ...
    return (<>
        ...
        <ReportChartCard>
          <Grid container justifyContent="center" spacing={8}>
            ...
            <Grid item xs={12} md={3}>
              <ReportCardBlue>
                <CardContent>
                  <h5><Currency value={this.state.netSummary.totalPurchasePayment} /></h5>
                  <p>Paid amount <br />*purchase in the period</p>
                </CardContent>
              </ReportCardBlue>
            </Grid>
            ...
            <Grid item xs={12} md={3}>
              <ReportCardPink>
                <CardContent>
                  <h5><Currency value={this.state.netSummary.totalbalanceAmount} /></h5>
                  <p>Balance <br />*purchase in the period</p>
                </CardContent>
              </ReportCardPink>
            </Grid>
            ...
            <Grid item xs={12} md={3}>
              <ReportCardGreen>
                <CardContent>
                  <h5><Currency value={this.state.netSummary.totalPaymentPaid} /></h5>
                  <p>Total amount paid</p>
                </CardContent>
              </ReportCardGreen>
            </Grid>
          </Grid>
          {this.state.dateWisePurchasePaymentsSummary && <>
            <PurchaseDatewiseChart title={`Purchase Trend ${formatDate(this.props.startDate)} -  ${formatDate(this.props.endDate)}`} data={this.state.dateWisePurchasePaymentsSummary} />
          </>}
        </ReportChartCard>
      }
      ...
      <Paper elevation={0}>
        ...
        <Grid container justifyContent="center" spacing={6}>
          ...
          {this.state.brandWiseSummary && <Grid item xs={12} md={5}>
            ...
            {(() => {
              let data: any = this.state.brandWiseSummary.map((r: any) => {
                return { name: r.productBrand, value: r.totalPurchase }
              });
          ...
          {/* Product catergorywise purchase */}
          {this.state.categoryWiseSummary && <Grid item xs={12} md={5}>
            ...
            {(() => {
              let data: any = this.state.categoryWiseSummary.map((r: any) => {
                return { name: r.productCategory, value: r.totalPurchase }
              });
            ...
            {(() => {
              ...
              return <ReportChartCardVertical>
                <ReportChartContainer>
                  ...
                  <ReportChartTitle>Productwise Purchase Trend (Top 20)</ReportChartTitle>
...
PurchaseReportsMetrics.contextType = RootContext;

export default PurchaseReportsMetrics;
